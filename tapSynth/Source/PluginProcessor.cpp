...
TapSynthAudioProcessor::TapSynthAudioProcessor()
     ...
     : AudioProcessor (BusesProperties()
                     ...
                     #endif
                       ), apvts(*dynamic_cast<juce::AudioProcessor*>(this), nullptr, "Parameters", createParams())
#endif
{
...
juce::AudioProcessorValueTreeState::ParameterLayout TapSynthAudioProcessor::createParams()
{
    ...
    params.push_back (std::make_unique<juce::AudioParameterFloat>(juce::ParameterID("OSC2FMFREQ", 1), "Oscillator 2 FM Frequency", juce::NormalisableRange<float> {0.0f, 1000.0f, 0.1f,}, 0.0f, "Hz"));
...
void TapSynthAudioProcessor::setVoiceParams()
{
    for (int i = 0; i < synth.getNumVoices(); ++i)
    {
        if (auto voice = dynamic_cast<SynthVoice*>(synth.getVoice(i)))
        {
            ...
            {
                osc1[j].setParams (osc1Choice, osc1Gain, osc1Pitch, osc1FmFreq, osc1FmDepth);
                osc2[j].setParams (osc2Choice, osc2Gain, osc2Pitch, osc2FmFreq, osc2FmDepth);
            }
